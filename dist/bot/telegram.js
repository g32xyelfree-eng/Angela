...
const whitelistMiddleware = async (ctx, next) => {
...
bot.on(['message:voice', 'message:audio'], async (ctx) => {
    ...
    try {
        ...
        console.log(`[Audio] Transcripción exitosa: "${transcribedText}"`);
        // Notificamos al usuario sobre la transcripción (opcional, pero útil)
        await ctx.reply(`_🎙️ ${transcribedText}_`, { parse_mode: 'Markdown' });
        // Volvemos a mostrar el indicador de que está "escribiendo..." la respuesta
        await ctx.replyWithChatAction('typing');
...
export const startBot = () => {
    console.log('Iniciando bot de Telegram (Long Polling)...');
    bot.start({
        onStart: (botInfo) => {
            console.log(`[Bot] Conectado exitosamente como @${botInfo.username}`);
        }
    });
    // Manejo elegante de las señales de apagado
